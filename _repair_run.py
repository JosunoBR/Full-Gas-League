"""
Script de reparo: sobrescreve o run.py corrompido com o conteúdo correto do servidor Flask.
Execute: py _repair_run.py
"""
import os

correct_content = '''from flask import Flask
...
# Mostra o que está lá antes
print("=== CONTEÚDO ATUAL do run.py (primeiras 5 linhas) ===")
try:
...
# Sobrescreve
print("\n=== SOBRESCREVENDO run.py ===")
with open(target, 'w', encoding='utf-8') as f:
    f.write(correct_content)

print(f"OK! run.py reescrito com {len(correct_content)} bytes.")
print("\n=== VERIFICAÇÃO (primeiras 5 linhas) ===")
with open(target, 'r', encoding='utf-8') as f:
    for i, line in enumerate(f, 1):
        print(f"  {i}: {line}", end='')
        if i >= 5:
...
print("\n\nPronto! Agora execute: py run.py")
